...
object Styles {
  ...
  def scrollViewStyle(implicit appContext: AppContext) =
    vMatchParent +
      vPaddings(8 dp)

  val horizontalLinearLayoutStyle = llHorizontal

  val verticalLinearLayoutStyle =
    vMatchParent +
      llVertical
  ...
  val tableLayoutStyle =
    vMatchWidth +
      llVertical

  def tableLayoutRowStyle(implicit appContext: AppContext) =
    vMatchWidth +
      llHorizontal +
